...
interface Filters {
    ...
    name?: { operator: FilterOperator; constraints: Array<{ value: string | null; matchMode: FilterMatchMode }> };
    'country.name'?: { operator: FilterOperator; constraints: Array<{ value: string | null; matchMode: FilterMatchMode }> };
    representative?: { value: string | null; matchMode: FilterMatchMode };
    date?: { operator: FilterOperator; constraints: Array<{ value: string | null; matchMode: FilterMatchMode }> };
    balance?: { operator: FilterOperator; constraints: Array<{ value: number | null; matchMode: FilterMatchMode }> };
    status?: { operator: FilterOperator; constraints: Array<{ value: string | null; matchMode: FilterMatchMode }> };
    activity?: { value: number[]; matchMode: FilterMatchMode };
    verified?: { value: boolean | null; matchMode: FilterMatchMode };
}

const customers1: Ref<Customer[] | null> = ref(null);
const customers2: Ref<Customer[] | null> = ref(null);
const customers3: Ref<Customer[] | null> = ref(null);
const filters1: Ref<Filters | null> = ref(null);
const loading1: Ref<boolean | null> = ref(null);
const balanceFrozen: Ref<boolean> = ref(false);
...
function getOrderSeverity(order: Order): string | null {
    switch (order.status) {
        case 'DELIVERED':
            return 'success';
...
function getStockSeverity(product: Product): string | null {
...
onBeforeMount(() => {
    ProductService.getProductsWithOrdersSmall().then((data: Product[]) => (products.value = data));
    ...
    CustomerService.getCustomersLarge().then((data: Customer[]) => (customers2.value = data));
    CustomerService.getCustomersMedium().then((data: Customer[]) => (customers3.value = data));
...
function expandAll(): void {
    if (products.value) {
        expandedRows.value = products.value.reduce((acc: Record<string, boolean>, p: Product) => {
            acc[p.id] = true;
            return acc;
        }, {});
    }
