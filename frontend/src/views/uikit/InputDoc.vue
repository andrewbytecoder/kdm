...
import { CountryService } from '../../service/CountryService.ts';
...
const floatValue: Ref<string | null> = ref(null);
const autoValue: Ref<Country[] | null> = ref(null);
const selectedAutoValue: Ref<Country | null> = ref(null);
const autoFilteredValue: Ref<Country[]> = ref([]);
const calendarValue: Ref<Date | null> = ref(null);
const inputNumberValue: Ref<number | null> = ref(null);
const sliderValue: Ref<number> = ref(50);
const ratingValue: Ref<number | null> = ref(null);
const colorValue: Ref<string> = ref('#1976D2');
const radioValue: Ref<string | null> = ref(null);
const checkboxValue: Ref<string[]> = ref([]);
const switchValue: Ref<boolean> = ref(false);
const listboxValues: Ref<Country[]> = ref([
...
const listboxValue: Ref<Country | null> = ref(null);
const dropdownValues: Ref<Country[]> = ref([
    { name: 'New York', code: 'NY' },
    { name: 'Rome', code: 'RM' },
    { name: 'London', code: 'LDN' },
    { name: 'Istanbul', code: 'IST' },
...
const dropdownValue: Ref<Country | null> = ref(null);
const multiselectValues: Ref<Country[]> = ref([
...
const multiselectValue: Ref<Country[] | null> = ref(null);
const toggleValue: Ref<boolean> = ref(false);
const selectButtonValue: Ref<{name: string} | null> = ref(null);
const selectButtonValues: Ref<{name: string}[]> = ref([{ name: 'Option 1' }, { name: 'Option 2' }, { name: 'Option 3' }]);
const knobValue: Ref<number> = ref(50);
const inputGroupValue: Ref<boolean> = ref(false);
const treeSelectNodes: Ref<TreeNode[] | null> = ref(null);
const selectedNode: Ref<TreeNode | null> = ref(null);

onMounted(() => {
    CountryService.getCountries().then((data: Country[]) => (autoValue.value = data));
    NodeService.getTreeNodes().then((data: TreeNode[]) => (treeSelectNodes.value = data));
});

function searchCountry(event: { query: string }): void {
    setTimeout(() => {
