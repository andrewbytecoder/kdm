...
const visibleLeft: Ref<boolean> = ref(false);
const visibleRight: Ref<boolean> = ref(false);
const visibleTop: Ref<boolean> = ref(false);
const visibleBottom: Ref<boolean> = ref(false);
const visibleFull: Ref<boolean> = ref(false);
const products: Ref<Product[] | null> = ref(null);
const selectedProduct: Ref<Product | null> = ref(null);
const op: Ref<any> = ref(null);
...
function onProductSelect(event: { data: Product }): void {
    op.value.hide();
    toast.add({ severity: 'info', summary: 'Product Selected', detail: event.data.name, life: 3000 });
}

function confirm(event: { target: HTMLElement }): void {
