...
interface Props {
    ...
    recent?: boolean;
    free?: boolean;
    containerClass?: string | string[] | Record<string, unknown>;
    previewStyle?: any;
}
...
const BlockView = reactive({
    PREVIEW: BlockViewEnum.PREVIEW,
    CODE: BlockViewEnum.CODE
});

const blockView: Ref<BlockViewEnum> = ref(BlockViewEnum.PREVIEW);
const codeCopied: Ref<boolean> = ref(false);
const codeCopyLoading: Ref<boolean> = ref(false);

function activateView(event: Event, blockViewValue: BlockViewEnum): void {
